...
const DashboardView = memo(({ user, isAdmin, loginKey, licenseKeys, configs, products, onToast, onOpenSettings, users }) => {
  const currentUser = users.find(u => u.user.toLowerCase() === user.toLowerCase());
...
const ChatView = ({ user, isAdmin, users, chats, setChats, onToast }) => {
  const [activeTab, setActiveTab] = useState(() => {
    if (!isAdmin) {
      const hasUnreadDm = (chats.dms[user] || []).some(m => !m.read && m.sender !== user);
      if (hasUnreadDm) return 'dm';
    }
    return 'global';
  })
  const [selectedUser, setSelectedUser] = useState('') // For admin DM selection
  ...
  const dmMessages = isAdmin 
    ? (chats.dms[selectedUser] || [])
    : (chats.dms[user] || [])

  // Mark DMs as read when opening the tab
  useEffect(() => {
    if (activeTab === 'dm') {
      const targetUser = isAdmin ? selectedUser : user
      if (targetUser && chats.dms[targetUser]) {
        const hasUnread = chats.dms[targetUser].some(m => !m.read && m.sender !== user)
        if (hasUnread) {
          setChats(prev => {
            const newDms = { ...prev.dms }
            newDms[targetUser] = newDms[targetUser].map(m => 
  ...
  }, [activeTab, selectedUser, user, isAdmin, chats.dms, setChats])
  ...
  const handleSend = (e) => {
    ...
    if (activeTab === 'global') {
      ...
      const targetUser = isAdmin ? selectedUser : user
      if (!targetUser) return onToast('Select a user to message')
      
      setChats(prev => {
        const newDms = { ...prev.dms }
        if (!newDms[targetUser]) newDms[targetUser] = []
        newDms[targetUser] = [...newDms[targetUser], newMsg]
        return { ...prev, dms: newDms }
      })
    }
    setMsgInput('')
  ...
  const renderMessages = (messages) => {
    return (
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 flex flex-col">
        {messages.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-[10px] uppercase tracking-[0.3em] text-zinc-700 font-black">
            No messages yet
          </div>
        ) : (
          messages.map(msg => {
            const isMe = msg.sender === user;
            const senderUser = users.find(u => u.user === msg.sender);
            const pfp = senderUser?.profilePic;
            
            return (
              <div key={msg.id} className={`flex gap-4 max-w-[80%] group ${isMe ? 'ml-auto flex-row-reverse' : ''}`}>
                <div className="w-10 h-10 shrink-0">
                  {pfp ? (
                    <img src={pfp} className="w-full h-full rounded-xl object-cover border border-white/10" alt="pfp" />
                  ) : (
                    <DefaultAvatar name={msg.sender} size="w-full h-full" />
                  )}
                </div>
                <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  <div className={`flex items-baseline gap-2 mb-1 ${isMe ? 'flex-row-reverse' : ''}`}>
                    <span className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">{msg.sender}</span>
                    <span className="text-[8px] text-zinc-600">{msg.timestamp}</span>
                    <button onClick={() => setReplyTo(msg)} className="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-white transition-opacity text-[10px] uppercase tracking-widest font-bold px-2">Reply</button>
                  </div>
                  <div className={`px-4 py-3 rounded-2xl text-sm ${isMe ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-zinc-900 border border-white/5 text-zinc-300 rounded-tl-sm'}`}>
                    {msg.replyTo && (
                      <div className={`mb-2 pl-2 border-l-2 text-xs opacity-70 ${isMe ? 'border-white/50 text-white' : 'border-zinc-500 text-zinc-400'}`}>
                        <div className="font-bold">{msg.replyTo.sender}</div>
                        <div className="truncate">{msg.replyTo.text || 'Attachment'}</div>
                      </div>
                    )}
                    {msg.attachment && (
                      <img src={msg.attachment} alt="attachment" className="max-w-[250px] rounded-lg mb-2 cursor-pointer hover:opacity-90 transition-opacity" />
                    )}
  ...
  return (
    <div className="dashboard-card h-[70vh] flex overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-1000">
      {/* Sidebar */}
      <div className="w-64 bg-zinc-950/50 border-r border-white/5 flex flex-col shrink-0">
        <div className="p-6 border-b border-white/5">
          <div className="text-[10px] uppercase tracking-[0.4em] text-white font-black mb-4">Chat Rooms</div>
          <button 
            onClick={() => setActiveTab('global')}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold text-[10px] uppercase tracking-widest ${activeTab === 'global' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'text-zinc-500 hover:bg-white/5'}`}
          >
          ...
          {!isAdmin ? (
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl mt-2 transition-all font-bold text-[10px] uppercase tracking-widest relative ${activeTab === 'dm' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'text-zinc-500 hover:bg-white/5'}`}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
              Admin Support
              {(chats.dms[user] || []).some(m => !m.read && m.sender !== user) && (
                <div className="absolute right-3 w-2 h-2 bg-red-500 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]"></div>
              )}
            </button>
          ) : (
            <div className="mt-8">
              <div className="text-[10px] uppercase tracking-[0.4em] text-white font-black mb-4">Direct Messages</div>
              <input 
                placeholder="Search user..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-zinc-900 border border-white/5 rounded-lg px-3 py-2 text-[10px] text-white mb-4 outline-none focus:border-blue-500/50"
              />
              <div className="space-y-1 overflow-y-auto max-h-[40vh] custom-scrollbar">
                {users
                  .filter(u => u.user.toLowerCase() !== user.toLowerCase() && u.user.toLowerCase().includes(searchQuery.toLowerCase()))
                  .map(u => {
                    const hasUnread = (chats.dms[u.user] || []).some(m => !m.read && m.sender !== user)
                    return (
                      <button
                        key={u.user}
                        onClick={() => { setActiveTab('dm'); setSelectedUser(u.user); }}
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all font-bold text-[10px] uppercase tracking-widest ${activeTab === 'dm' && selectedUser === u.user ? 'bg-white/10 text-white' : 'text-zinc-500 hover:bg-white/5'}`}
                      >
                        <span className="truncate">{u.user}</span>
                        {hasUnread && <div className="w-2 h-2 bg-red-500 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)] shrink-0"></div>}
                      </button>
                    )
                  })}
              </div>
      ...
      <div className="flex-1 flex flex-col bg-zinc-900/20 relative">
        {/* Header */}
        <div className="h-16 border-b border-white/5 flex items-center px-8 shrink-0 bg-zinc-950/40">
          <div className="text-sm font-bold text-white tracking-widest uppercase flex items-center gap-3">
            <span className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]"></span>
            {activeTab === 'global' ? 'Global Channel' : (isAdmin ? `DM: ${selectedUser || 'Select a user'}` : 'Admin Support')}
          </div>
        </div>

        {/* Messages */}
        {activeTab === 'global' ? renderMessages(chats.global || []) : (isAdmin && !selectedUser ? (
          <div className="flex-1 flex items-center justify-center text-[10px] uppercase tracking-widest text-zinc-600 font-bold">
            Select a user to view messages
          </div>
        ) : renderMessages(dmMessages))}

        {/* Input */}
        {!(activeTab === 'dm' && isAdmin && !selectedUser) && (
          <div className="bg-zinc-950/60 border-t border-white/5 shrink-0 flex flex-col">
            {(replyTo || attachment) && (
              <div className="px-4 py-2 border-b border-white/5 bg-zinc-900/40 flex items-center justify-between text-xs text-zinc-400">
                <div className="flex items-center gap-4">
                  {replyTo && (
                    <div className="flex items-center gap-2">
                      <span className="text-blue-400">Replying to {replyTo.sender}:</span>
                      <span className="truncate max-w-[200px]">{replyTo.text || 'Attachment'}</span>
                    </div>
                  )}
                  {attachment && (
                    <div className="flex items-center gap-2">
                ...
                <button onClick={() => { setReplyTo(null); setAttachment(null); }} className="hover:text-white">
            ...
            <form onSubmit={handleSend} className="p-4 flex items-center gap-3">
              ...
              <button 
                ...
                className="p-3 bg-zinc-900 hover:bg-zinc-800 border border-white/10 rounded-xl text-zinc-400 hover:text-white transition-colors shrink-0"
              >
              ...
              <div className="relative flex-1 flex items-center">
                <input 
                  ...
                  className="w-full bg-zinc-900 border border-white/10 rounded-xl pl-4 pr-12 py-4 text-sm text-white outline-none focus:border-blue-500/50 transition-colors"
                />
                <button 
                  type="submit"
...
const SettingsView = ({ user, isAdmin, loginKey, licenseKeys, onToast, users, setUsers, alwaysLoggedIn, setAlwaysLoggedIn, setSavedCredentials }) => {
    const fileInputRef = useRef(null)
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      const reader = new FileReader()
      reader.onloadend = () => {
        setUsers(users.map(u => u.user.toLowerCase() === user.toLowerCase() ? { ...u, profilePic: reader.result } : u));
        if (isAdmin && !currentUser) {
           setUsers([...users, { user: user, password: '', profilePic: reader.result }]);
        }
...
const Dashboard = ({ user, isAdmin, loginKey, products, setProducts, configs, setConfigs, licenseKeys, setLicenseKeys, users, setUsers, alwaysLoggedIn, setAlwaysLoggedIn, setSavedCredentials, onLogout, chats, setChats }) => {
    ...
    <div className={`w-full ${activeTab === 'CONFIGS' ? 'max-w-7xl' : 'max-w-5xl'} px-4 relative z-10 flex flex-col min-h-[90vh] transition-all duration-700`}>
      ...
      <div className="flex-1 pb-32">
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === 'DASHBOARD' && <DashboardView user={user} isAdmin={isAdmin} loginKey={loginKey} licenseKeys={licenseKeys} configs={configs} products={products} onToast={showToast} onOpenSettings={() => setActiveTab('SETTINGS')} users={users} />}
            ...
            {activeTab === 'CHAT' && <ChatView user={user} isAdmin={isAdmin} users={users} chats={chats} setChats={setChats} onToast={showToast} />}
            {activeTab === 'SETTINGS' && <SettingsView user={user} isAdmin={isAdmin} loginKey={loginKey} licenseKeys={licenseKeys} onToast={showToast} users={users} setUsers={setUsers} alwaysLoggedIn={alwaysLoggedIn} setAlwaysLoggedIn={setAlwaysLoggedIn} setSavedCredentials={setSavedCredentials} />}
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-fit px-4 z-[100]">
        <div className="flex items-center gap-1 p-1.5 bg-black/60 border border-white/10 rounded-2xl backdrop-blur-2xl shadow-2xl mx-auto">
          ...
          <button onClick={() => setActiveTab('CHAT')} className={`nav-tab px-6 py-3 relative ${activeTab === 'CHAT' ? 'active' : ''}`}>
            ...
            {(() => {
              ...
              let hasUnread = false;
              if (isAdmin) {
                hasUnread = Object.values(chats?.dms || {}).some(msgs => msgs.some(m => !m.read && m.sender !== user));
              } else {
                hasUnread = (chats?.dms?.[user] || []).some(m => !m.read && m.sender !== user);
              }
              return hasUnread ? <div className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)] border border-black" /> : null;
            })()}
          </button>
...
function App() {
  ...
  const [users, setUsers] = usePersistedState('azov_users', [
    { user: 'alex', password: 'password123', profilePic: null }
  ])
  const [chats, setChats] = usePersistedState('azov_chats', { global: [], dms: {} })
